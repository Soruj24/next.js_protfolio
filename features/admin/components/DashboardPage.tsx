...
import { FolderKanban, Code2, Mail, Gauge, FileText, User } from "lucide-react";
...
import {
  ...
  WidgetSkeleton,
  SectionSkeleton,
} from "@/features/admin/components/dashboard/skeletons";
...
const ActivityFeed = dynamic(() => import("./command-center/ActivityFeed"), {
  ssr: false,
  loading: () => <WidgetSkeleton />,
});
const QuickActions = dynamic(() => import("./command-center/QuickActions"), {
  ssr: false,
  loading: () => <WidgetSkeleton />,
});
const SystemStatus = dynamic(() => import("./command-center/SystemStatus"), {
  ssr: false,
  loading: () => <WidgetSkeleton />,
});
...
const RecentCommits = dynamic(() => import("./command-center/RecentCommits"), {
  ssr: false,
  loading: () => <WidgetSkeleton />,
});
const PinnedActions = dynamic(() => import("./command-center/PinnedActions"), {
  ssr: false,
...
export default function DashboardPage({ data }: { data: DashboardData }) {
  ...
  return (
    <div className="space-y-6" role="region" aria-label="Dashboard">
      <div
        className="absolute -top-24 -left-24 w-72 h-72 bg-cyan-500/[0.06] blur-[120px] rounded-full pointer-events-none"
        aria-hidden="true"
      />
      <div
        className="absolute top-1/2 -right-32 w-96 h-96 bg-purple-500/[0.04] blur-[140px] rounded-full pointer-events-none"
        aria-hidden="true"
      />
      ...
      <div className="relative animate-fade-in-up">
        <div className="relative">
          <div className="flex items-center gap-2 mb-2">
            <div
              className="w-2 h-2 rounded-full bg-emerald-400 animate-status-pulse"
              aria-hidden="true"
            />
          ...
          {currentTime && (
            <p className="text-xs text-gray-600 font-semibold mt-2 uppercase tracking-wider">
              {currentTime}
            </p>
          )}
        </div>
      ...
      >
        ...
        {/*
          FIXED: was a plain text <Link> sitting inside a grid of StatCards,
          which broke row height/alignment. Now styled to match the same
          dark glassmorphism card language (border, bg-white/[0.03], icon
          chip, hover glow) used by the rest of the grid.
        */}
        <Link
          href="/admin/resume"
          className="group relative rounded-xl border border-white/[0.08] bg-white/[0.03] p-4 flex flex-col justify-between hover:border-cyan-400/30 hover:bg-white/[0.05] transition-colors"
        >
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">Edit &amp; download CV</p>
          </div>
        </Link>
        ...
        <StatCard
          ...
          trend={
            data.blogs.total > data.blogs.published
              ? `${data.blogs.total - data.blogs.published} drafts`
              : "All published"
          }
      ...
      <div
        className="grid grid-cols-1 lg:grid-cols-12 gap-4 animate-fade-in-up"
