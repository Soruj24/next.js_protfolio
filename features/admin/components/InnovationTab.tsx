...
export default function InnovationTab() {
  ...
  const { fields, append, remove } = useFieldArray({ control, name: "innovation" });
    ...
    <>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Button
          ...
          variant="outline"
          size="sm"
          onClick={() => append({ title: "", description: "", icon: "", color: "" })}
      ...
      {fields.map((field, index) => (
        <Card key={field.id} className="bg-gray-800 border-gray-700 shadow-lg overflow-hidden">
          ...
          <CardContent className="p-4 sm:p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField control={control} name={`innovation.${index}.title`} render={({ field }) => (
            ...
            <FormField control={control} name={`innovation.${index}.icon`} render={({ field }) => (
            ...
            <FormField control={control} name={`innovation.${index}.color`} render={({ field }) => (
            ...
            <FormField control={control} name={`innovation.${index}.description`} render={({ field }) => (
