...
interface Burst { x: number; y: number; radius: number; maxRadius: number; opacity: number; color: string; }

interface Settings { layer0Count: number; layer1Count: number; connectDist: number; gridSize: number; }

function getSettings(w: number, rm: boolean): Settings {
  if (rm) {
